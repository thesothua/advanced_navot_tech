...
@section('content')
    <div class="container py-5">
        ...
        <nav aria-label="breadcrumb" class="mb-4" data-aos="fade-up">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('products') }}" class="text-decoration-none text-danger">Products</a></li>
                        ...
                        <div class="carousel-inner rounded-3 shadow-sm">
                            @foreach($images as $index => $image)
                                <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                                    <img src="{{ $image->getUrl() }}" class="d-block w-100 product-main-image" alt="{{ $product->name }}">
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="col-lg-6" data-aos="fade-left">
                ...
                <div class="mb-4">
                    @if($product->sale_price)
                        <span class="text-decoration-line-through text-muted me-3 fs-5">₹{{ number_format($product->price, 2) }}</span>
                            ...
                            <div class="col-md-6 col-lg-3">
                                <div class="card h-100 border-0 shadow-sm rounded-3 product-card">
                                    ...
                                    <div class="card-img-container">
                                        @if($media)
                                            <img src="{{ $media->getUrl() }}" class="card-img-top rounded-top" alt="{{ $relatedProduct->name }}">
                                        @else
                                            <img src="https://via.placeholder.com/300x200?text={{ urlencode($relatedProduct->name) }}" class="card-img-top rounded-top" alt="{{ $relatedProduct->name }}">
                                        @endif
                                    ...
                                    <div class="card-body p-3">
                                        ...
                                        @endif
                                        <h5 class="card-title fw-bold mb-2">{{ Str::limit($relatedProduct->name, 30) }}</h5>
                                        <div class="mb-2">
                                            @if($relatedProduct->sale_price)
                                    ...
                                    <div class="card-footer bg-white border-top-0 p-3">
                                        <a href="{{ route('products.show', $relatedProduct->slug) }}" class="btn btn-outline-danger btn-sm">View Details</a>
                                    </div>
                                </div>
                            </div>
                        @endforeach
